...
ecorrap_named_locations <- c(
    "Aukane Island", "Davies Reef", "Dungeness Reef", "Halfway Island",
    "Heron Island", "Lady Musgrave Island", "Lizard Island", "Masi Island", 
    "Moore", "Orpheus", "Pelorus")
...
#obtain partial matches
partial_matches <- purrr::map(ecorrap_named_locations, ~ unique(str_extract(named_reefs_and_islands$GBR_NAME, .x)))

#drop the irrelevant Na values in the first column
partial_matches <- purrr::map(partial_matches, ~ .x[2])

#collapse the named locations into a single string
names <- str_c(ecorrap_named_locations, collapse = "|")

#filter the named reefs and islands dataset to only include the names we are interested in
ecorrap_named_locations <- named_reefs_and_islands |> 
    filter(str_detect(GBR_NAME, names))

st_write(ecorrap_named_locations, "ecorrap_orthomosaic_sites.shp")
